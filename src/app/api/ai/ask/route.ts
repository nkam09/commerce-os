...
import { answerBusinessQuestion, buildCompanyContextPack } from "@/lib/ai-service";

export async function POST(req: NextRequest) {
  try {
    const user = await getUserFromRequest(req);
    const { question, marketplaceId, startingCash } = await req.json();
    ...
    const contextPack = buildCompanyContextPack({
      ...
      productSummaries: overview.products.map(({ product, summary }: any) => ({
        sku:          product.sku,
        netProfit:    summary?.netProfit ?? 0,
        healthStatus: "HEALTHY",
      })),
      cashForecast: cashForecast.map(m => ({
        month:           m.month,
        endingCash:      m.endingCash,
        cashFloorBreach: m.cashFloorBreach,
      })),
      openCriticalAlerts: overview.alerts.filter((a: any) => a.severity === "HIGH").length,
      alertTitles:        overview.alerts.map((a: any) => a.title),
    });

    const answer = await answerBusinessQuestion(question, contextPack);
