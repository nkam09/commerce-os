...
export async function getTrendsViewData(userId: string, brand?: string): Promise<TrendsViewData> {
  ...
  for (const row of perProdFees) {
    ...
    if (!prodBuckets.has(k)) prodBuckets.set(k, { grossSales: 0, refunds: 0, unitsSold: 0, orderCount: 0, totalFees: 0, adSpend: 0, adSales: 0 });
    const b = prodBuckets.get(k)!;
    b.totalFees += toNum(row._sum.referralFee) + toNum(row._sum.fbaFee) + toNum(row._sum.storageFee) + toNum(row._sum.awdStorageFee) + toNum(row._sum.returnProcessingFee) + toNum(row._sum.otherFees) - toNum(row._sum.reimbursement);
  }
